...
export function passwordResetMessage(baseURL: string, recipient: string, token: string): MailMessage {
...
export function queuePasswordResetEmail(mailer: Mailer, baseURL: string, recipient: string, token: string): void {
  const message = passwordResetMessage(baseURL, recipient, token);
