...
export function PasswordField({ label, aside, id: suppliedId, ...input }: Omit<InputHTMLAttributes<HTMLInputElement>, 'type'> & {
  ...
  return <div className="password-field">
    <div className="password-label-row"><label htmlFor={id}>{label}</label>{aside}</div>
    <div className="password-control"><input {...input} id={id} type={visible ? 'text' : 'password'}
      ...
      <button type="button" className="password-visibility" aria-label={passwordVisibilityLabel(visible)}
